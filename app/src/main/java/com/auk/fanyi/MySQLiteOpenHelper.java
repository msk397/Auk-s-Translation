...
public class MySQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String sql = "create table history(id int primary key,yuan varchar(200),mudi varchar(200),star int)";
        //execSQL用于执行SQL语句
        //完成数据库的创建
        db.execSQL(sql);
        //数据库实际上是没有被创建或者打开的，直到getWritableDatabase() 或者 getReadableDatabase() 方法中的一个被调用时才会进行创建或者打开
        sql = "create table starsql(id int,yuan varchar(200),mudi varchar(200),star int)";
        //execSQL用于执行SQL语句
        //完成数据库的创建
        db.execSQL(sql);
    }
